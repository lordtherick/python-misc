...
def treeSize(server,token,pageItem,parentPath,depth,doChildPages):
	...
	pageid = pageItem['id']
	directorySize = attachmentsSize(server,token,pageid,depth)
	...
	if doChildPages == 1:
		for pagedict in childPages:
			...
			directorySize += treeSize(server,token,pagedict,parentPath,depth,doChildPages)
			parentPath.pop()
	...
	if directorySize > 0:
		...
		if pageInfo:
			...
			if 'modified' in pageInfo: modified = pageInfo['modified']
...
print ("total space size="+GetHumanReadable(spaceSize,4))
